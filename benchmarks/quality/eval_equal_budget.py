...
@torch.no_grad()
def answer(model, tok, context, city, code, block_size, max_new_tokens, do_recall):
    ...
    ctx_len = len(ids)
    if do_recall:
        sub = tok(f" {code}", add_special_tokens=False).input_ids
        blocks = find_block_ids(ids, sub, block_size)
        if not blocks:  # fall back to the code without leading space
            blocks = find_block_ids(ids, tok(code, add_special_tokens=False).input_ids, block_size)
        set_recall_target(blocks)
    out = model.generate(**inputs, max_new_tokens=max_new_tokens, do_sample=False)
    text = tok.decode(out[0, ctx_len:], skip_special_tokens=True)
    rec = get_recall() if do_recall else float("nan")
    if do_recall:
        clear_recall_target()
    return (code in text), rec, ctx_len
...
def eval_method(model, tok, data, method, budget, common, block_size, max_new_tokens):
    ...
    for prompt, cities, codes in data:
        for city, code in zip(cities, codes):
            ok, rec, ctx_len = answer(model, tok, prompt, city, code,
                                      block_size, max_new_tokens, is_sparse)
...
def main():
    ...
    print(f"\nmulti-key NIAH | ctx≈{args.ctx} tok | {args.needles} needles x "
          f"{args.trials} trials = {n_q} queries/cell | sink {args.n_sink} "
          f"local {args.n_local} c_min {args.c_min}\n")
    ...
    if "full" in args.methods:
        ...
        rows.append(dict(method="full", target_budget="", realized_budget="",
                         recall="", accuracy=acc, ctx_tokens=ctx, queries=n_q))
    ...
    for b in args.budgets:
        for m in args.methods:
            ...
            rows.append(dict(method=m, target_budget=b, realized_budget=round(real, 3),
                             recall=round(rec, 3), accuracy=acc, ctx_tokens=ctx, queries=n_q))
        print()
