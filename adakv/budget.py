...
def allocate_budget(
    ...
    """Allocate a per-head block budget from a global average.

    Args:
        scores: ``[n_heads, n_blocks]`` block importance scores.
        avg_budget: target *mean* number of blocks per head. The global KV
            budget is ``avg_budget * n_heads * block_size`` tokens.
    ...
    Returns:
        ``[n_heads]`` int64 budget per head, clamped to ``[k_min, k_max]`` and
        rescaled so ``mean(k) ~= avg_budget``.
    """
    H, B = scores.shape
    if B <= 1:
